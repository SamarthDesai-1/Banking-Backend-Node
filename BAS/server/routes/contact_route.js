...
route.post("/send-user-query", verifyArray, (request, response, next) => {
  ...
  if (result.errors.length == 0) {
    ...
    return response.status(402).send({ msg: "First fill the form for contact" });
  }
  else if (result.errors[0].path == "email") {
    return response.status(402).send({ msg: "Please enter valid email" });
  }
  else if (result.errors[0].path == "name") {
    return response.status(402).send({ msg: "Your name should only contains Alphabetically letters" });
  }
  else if (result.errors[0].path == "mobile") {
    return response.status(402).send({ msg: "Please enter valid mobile" });
  }
  else if (result.errors[0].path == "subject") {
    return response.status(402).send({ msg: "Subject not should be empty and minimum 10 characters are required" });
  }
  else if (result.errors[0].path == "message") {
    return response.status(402).send({ msg: "Message not should be empty and minimum 20 characters are required" });
  }
